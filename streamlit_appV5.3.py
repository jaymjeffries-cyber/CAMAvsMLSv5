...
COLUMNS_TO_COMPARE_SUM = [
    {'mls_col': 'Below Grade Finished Area', 'cama_cols': ['TOTAL_BASEMENT_FIN']}
]
...
def lookup_zone(taxdist_val):
    """Return Zone for a TAXDIST value, normalizing to 5-digit zero-padded string."""
    if pd.isna(taxdist_val) or str(taxdist_val).strip() == '':
        return 'Unknown'
    try:
        key = str(int(float(str(taxdist_val).strip()))).zfill(5)
    except (ValueError, TypeError):
        key = str(taxdist_val).strip().zfill(5)
...
def compare_data_enhanced(df_mls, df_cama, unique_id_col, cols_to_compare_mapping,
    ...
    for _, row in merged_df.iterrows():
        ...
        if ms == 'left_only':
            ...
            taxdist=row.get('TAXDIST',''); zone=lookup_zone(taxdist)
            rmm=[]; fc=[]
            ...
            if cols_to_compare_sum:
                for m in cols_to_compare_sum:
                    ...
                    if not values_equal(mv, csum):
                        rmm.append({'Parcel_ID':rid,'NOPAR':np_,'ADDITIONAL_PARCELS':ap,'Listing_Number':ln,'SALEKEY':sk,'Address':addr,'City':city,'State':state,'Zip':zc,'Zone':zone,'Field_MLS':mc,'Field_CAMA':f"SUM({', '.join(ccs)})",'MLS_Value':mv,'CAMA_Value':csum,'Difference':calculate_difference(mv,csum),'Parcel_URL':pu.format(parcel_id=rid) if pu else '','Zillow_URL':format_zillow_url(addr,city,state,zc)})
            if cols_to_compare_categorical:
                for m in cols_to_compare_categorical:
                    ...
                    if not im:
                        rmm.append({'Parcel_ID':rid,'NOPAR':np_,'ADDITIONAL_PARCELS':ap,'Listing_Number':ln,'SALEKEY':sk,'Address':addr,'City':city,'State':state,'Zip':zc,'Zone':zone,'Field_MLS':mc,'Field_CAMA':cc,'MLS_Value':mv,'CAMA_Value':cv,'Expected_CAMA_Value':ec,'Match_Rule':f"If '{ct}' in {mc}, then {cc} should be {m.get('cama_expected_if_true')}, else {m.get('cama_expected_if_false')}",'Parcel_URL':pu.format(parcel_id=rid) if pu else '','Zillow_URL':format_zillow_url(addr,city,state,zc)})
            if not rmm and fc:
                pm.append({'Parcel_ID':rid,'NOPAR':np_,'ADDITIONAL_PARCELS':ap,'Listing_Number':ln,'SALEKEY':sk,'Address':addr,'City':city,'State':state,'Zip':zc,'Zone':zone,'Fields_Compared':len(fc),'Fields_List':', '.join(fc),'Parcel_URL':pu.format(parcel_id=rid) if pu else '','Zillow_URL':format_zillow_url(addr,city,state,zc)})
            vm.extend(rmm)
...
if mls_file and cama_file:
    ...
    with st.expander("ℹ️ Expected Data Format"):
        ...
        **CAMA Data:** `PARID` · `NOPAR` · `CITYNAME` · `TAXDIST` · `SFLA` · `RMBED` · `FIXBATH` · `FIXHALF` · `TOTAL_BASEMENT_FIN` · `HEAT` · `SALEKEY`
        """)
