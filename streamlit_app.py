    ...
    if st.session_state.get('comparison_complete', False):
        ...
        if not df_perfect_matches.empty or not df_value_mismatches.empty:
            with st.expander("⚙️ Generate CAMA Mass Update Files", expanded=False):
                st.markdown("""
                This section generates two files for mass updating the CAMA system:
                ...
                with col1:
                    user_initials = st.text_input(
                        "Your Initials (e.g., JMJ)", 
                        value="JMJ",
                        help="Used for USER1 field in SALETAB"
                    )
                
                with col2:
                    user_full_name = st.text_input(
                        "Your Full Name (e.g., Jason Jeffries)",
                        value="Jason Jeffries",
                        help="Used for appraiser and Last Changed By fields in MassEntrance"
                    )
                    ...
                    if combined_df.empty:
                        ...
                        with col1:
                            st.download_button(
                                ...
                                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                help="Excel file for updating SALE tab in CAMA"
                            )
                        ...
                        with col2:
                            st.download_button(
                                ...
                                mime="text/csv",
                                help="CSV file for updating ENTRANCE in CAMA"
                            )
        else:
